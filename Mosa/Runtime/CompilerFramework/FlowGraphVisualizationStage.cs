...
namespace Mosa.Runtime.CompilerFramework
{
    ...
    public class FlowGraphVisualizationStage : BaseStage, IMethodCompilerStage, IPipelineStage
    {
        ...
        /// 
        /// </summary>
        protected static Dictionary<string, int> methodCount = new Dictionary<string, int> ();
        /// <summary>
        /// 
        /// </summary>
        ...
        private static PipelineStageOrder[] _pipelineOrder = new PipelineStageOrder[] { new PipelineStageOrder (PipelineStageOrder.Location.Before, typeof(IR.CILTransformationStage)) };
            ...
            foreach (BasicBlock block in BasicBlocks)
            {
                ...
                for (Context ctx = new Context (InstructionSet, block); !ctx.EndOfInstruction; ctx.GotoNext ())
                {
                    ...
                    if (inst.StartsWith ("IL") || inst.StartsWith ("T_"))
                        color = "#0000ff5f"; else if (inst.StartsWith ("IR"))
                        color = "#ff00005f";
                    else
                        color = "#CFD6CEff";
