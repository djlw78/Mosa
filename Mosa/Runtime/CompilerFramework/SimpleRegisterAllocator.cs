...
namespace Mosa.Runtime.CompilerFramework
{
    ...
    public sealed class SimpleRegisterAllocator : BaseStage, IMethodCompilerStage, IPipelineStage
    {
        ...
        private void AssignRegisters (Context ctx)
        {
            ...
            IRegisterConstraint rc = null;
            // _architecture.GetRegisterConstraint(instr);  // FIXME PG - pass context instead???
            // Operand index
            int opIdx;
            ...
            if (rc != null)
            {
                ...
                foreach (Operand op in ctx.Operands)
                {
                    ...
                    if (!rc.IsValidOperand (opIdx, op))
                    {
                        ...
                        ctx.SetOperand (opIdx, rop);
                        // FIXME PG - opIdx? hmmm. is this phidata?
                    }
        ...
        private RegisterOperand SpillRegister (Context ctx, SigType type, Register[] regs)
        {
            ...
            Array.Sort<KeyValuePair<Register, Context>> (lastUses, delegate(KeyValuePair<Register, Context> a, KeyValuePair<Register, Context> b) { return b.Value.Offset - a.Value.Offset; });
