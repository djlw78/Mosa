...
namespace Mosa.Runtime.Metadata
{
    ...
    public sealed class TableHeap : Heap
    {
        ...
        private static readonly int[][] IndexTables = new[] {
            new[] {
                (int)TokenTypes.TypeDef,
                (int)TokenTypes.TypeRef,
                (int)TokenTypes.TypeSpec
            },
            new[] {
                (int)TokenTypes.Field,
                (int)TokenTypes.Param,
                (int)TokenTypes.Property
            },
            new[] {
                (int)TokenTypes.MethodDef,
                (int)TokenTypes.Field,
                (int)TokenTypes.TypeRef,
                (int)TokenTypes.TypeDef,
                (int)TokenTypes.Param,
                (int)TokenTypes.InterfaceImpl,
                (int)TokenTypes.MemberRef,
                (int)TokenTypes.Module,
                /*(int)TokenTypes.Permission,*/
                (int)TokenTypes.Property,
                (int)TokenTypes.Event,
                (int)TokenTypes.StandAloneSig,
                (int)TokenTypes.ModuleRef,
                (int)TokenTypes.TypeSpec,
                (int)TokenTypes.Assembly,
                (int)TokenTypes.AssemblyRef,
                (int)TokenTypes.File,
                (int)TokenTypes.ExportedType,
                (int)TokenTypes.ManifestResource
            },
            new[] {
                (int)TokenTypes.Field,
                (int)TokenTypes.Param
        ...
        public TableHeap (IMetadataProvider provider, byte[] metadata, int offset, int size) : base(metadata, offset, size)
        {
        ...
        public void Read (TokenTypes token, out FieldLayoutRow result)
        {
            ...
            using (BinaryReader reader = CreateReaderForToken (token))
            {
                result = new FieldLayoutRow (reader.ReadUInt32 (), ReadIndexValue (reader, TokenTypes.Field));
            }
        }
        ...
        public void Read (TokenTypes token, out EventMapRow result)
        {
            ...
            using (BinaryReader reader = CreateReaderForToken (token))
            {
                result = new EventMapRow (ReadIndexValue (reader, TokenTypes.TypeDef), ReadIndexValue (reader, TokenTypes.Event));
            }
        }
        ...
        public void Read (TokenTypes token, out PropertyMapRow result)
        {
            ...
            using (BinaryReader reader = CreateReaderForToken (token))
            {
                result = new PropertyMapRow (ReadIndexValue (reader, TokenTypes.TypeDef), ReadIndexValue (reader, TokenTypes.Property));
            }
        }
        ...
        public void Read (TokenTypes token, out MethodImplRow result)
        {
            ...
            using (BinaryReader reader = CreateReaderForToken (token))
            {
                result = new MethodImplRow (ReadIndexValue (reader, TokenTypes.TypeDef), ReadIndexValue (reader, IndexType.MethodDefOrRef), ReadIndexValue (reader, IndexType.MethodDefOrRef));
            }
        }
        ...
        public void Read (TokenTypes token, out ModuleRefRow result)
        {
            ...
            using (BinaryReader reader = CreateReaderForToken (token))
            {
                result = new ModuleRefRow (ReadIndexValue (reader, IndexType.StringHeap));
            }
        }
        ...
        public void Read (TokenTypes token, out AssemblyProcessorRow result)
        {
            ...
            using (BinaryReader reader = CreateReaderForToken (token))
            {
                result = new AssemblyProcessorRow (reader.ReadUInt32 ());
            }
        }
        ...
        public void Read (TokenTypes token, out AssemblyRefOSRow result)
        {
            ...
            using (BinaryReader reader = CreateReaderForToken (token))
            {
                result = new AssemblyRefOSRow (reader.ReadUInt32 (), reader.ReadUInt32 (), reader.ReadUInt32 (), ReadIndexValue (reader, TokenTypes.AssemblyRef));
            }
        }
        ...
        public void Read (TokenTypes token, out NestedClassRow result)
        {
            ...
            using (BinaryReader reader = CreateReaderForToken (token))
            {
                result = new NestedClassRow (ReadIndexValue (reader, TokenTypes.TypeDef), ReadIndexValue (reader, TokenTypes.TypeDef));
            }
        }
