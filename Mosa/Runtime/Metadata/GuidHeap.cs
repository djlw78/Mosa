...
namespace Mosa.Runtime.Metadata
{
    ...
    public sealed class GuidHeap : Heap
    {
        ...
        public GuidHeap (byte[] metadata, int offset, int size) : base(metadata, offset, size)
        {
        }
        ...
        public Guid ReadGuid (ref TokenTypes token)
        {
            ...
            return new Guid (BitConverter.ToInt32 (buffer, realOffset), BitConverter.ToInt16 (buffer, realOffset + 4), BitConverter.ToInt16 (buffer, realOffset + 6), buffer[realOffset + 8], buffer[realOffset + 9], buffer[realOffset + 10], buffer[realOffset + 11], buffer[realOffset + 12], buffer[realOffset + 13], buffer[realOffset + 14],
            buffer[realOffset + 8]);
        }
